/* This function "t" tests the function "f" to see if computing at
   two dfferent scales has much effect on the accuracy. 
   test from f(x) to f(y) incrementing the index by(d.  f(i) is
   computed at two scales, scale s and then scale t, where t>s1.
   ...
   (called index) and the two original values val1 (scale s) andõ   val2 (scale t) */
